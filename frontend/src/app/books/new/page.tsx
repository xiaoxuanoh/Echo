...
export default async function NewBookPage({
  ...
  searchParams: Promise<{ folderId?: string; folderTitle?: string }>;
}) {
  const { folderId, folderTitle } = await searchParams;
      ...
      <div className="mx-auto max-w-6xl">
        ...
        <header className="mt-7 max-w-3xl">
          <p className="text-sm font-bold tracking-[0.16em] text-accent uppercase">
            {isAddingRecording ? "New recording" : "New book"}
          </p>
          <h1 className="mt-3 text-4xl font-semibold tracking-[-0.025em] sm:text-5xl">
            {isAddingRecording ? "Upload more pages" : "Upload your book"}
          </h1>
          <p className="mt-4 text-lg leading-8 text-muted">
            {isAddingRecording
              ? "Choose another PDF or more page photos. Echo will save this as a separate recording inside the selected book."
        ...
        <section className="mt-10 rounded-3xl border border-border bg-surface p-5 shadow-[0_20px_60px_rgba(48,55,61,0.06)] sm:p-8">
          <BookUpload libraryBookId={folderId} libraryBookTitle={folderTitle} />
        </section>
      </div>
    </main>
  );
